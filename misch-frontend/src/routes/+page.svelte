<script lang="ts">
	...
	const defaultExample = exampleProgramService.getById(DEFAULT_EXAMPLE_PROGRAM_ID);
	...
	const selectedExampleId = $derived<ExampleProgramId | 'custom'>(
		exampleProgramService.findMatchingExampleId(assembly, paperTapeInput) ?? 'custom'
	);
