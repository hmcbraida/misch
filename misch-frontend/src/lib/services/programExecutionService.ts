...
export class ProgramExecutionService {
	...
	async runToCompletion(request: RunProgramRequest): Promise<string> {
		...
		try {
			...
			const output = await this.sessionsClient.getOutputText(sessionId, this.config.outputUnit);
			return this.wrapOutput(output.units[String(this.config.outputUnit)] ?? '');
		} finally {
			if (sessionId) {
				try {
